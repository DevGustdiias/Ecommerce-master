				...
				<div class='col-md-4'>
							<div class='panel panel-info'>
								...
								<div class='panel-heading'>".CURRENCY." $pro_price.00
									<button pid='$pro_id' style='float:right;' id='product' class='btn btn-danger btn-xs'>Adicionar ao carrinho</button>
								</div>
							</div>
						</div>	
			";
		...
		if($count > 0){
			echo "
				<div class='alert alert-warning'>
						...
						<b>O produto já foi adicionado ao carrinho. Continue comprando..!</b>
				</div>
			";
		} else {
			...
			if (mysqli_num_rows($query) > 0) {
				echo "
					<div class='alert alert-warning'>
							<a href='#' class='close' data-dismiss='alert' aria-label='close'>&times;</a>
							<b>O produto já foi adicionado ao carrinho. Continue comprando..!
					</div>";
					exit();
			}
			$sql = "INSERT INTO `cart`
			...
			if (mysqli_query($con,$sql)) {
				echo "
					<div class='alert alert-success'>
						<a href='#' class='close' data-dismiss='alert' aria-label='close'>&times;</a>
						<b>Seu produto foi adicionado com sucesso..!</b>
					</div>
				";
				exit();
			}
...
//Contar item do carrinho do usuário
if (isset($_POST["count_item"])) {
	//Quando o usuário estiver logado, contaremos o número de itens no carrinho usando o ID da sessão do usuário
	if (isset($_SESSION["uid"])) {
		$sql = "SELECT COUNT(*) AS count_item FROM cart WHERE user_id = $_SESSION[uid]";
	}else{
		//Quando o usuário não estiver logado, contaremos o número de itens no carrinho usando o endereço IP exclusivo do usuário
...
//Obter item do carrinho do banco de dados para o menu suspenso
if (isset($_POST["Common"])) {

	if (isset($_SESSION["uid"])) {
		//Quando o usuário estiver logado, esta consulta será executada
		$sql = "SELECT a.product_id,a.product_title,a.product_price,a.product_image,b.id,b.qty FROM products a,cart b WHERE a.product_id=b.p_id AND b.user_id='$_SESSION[uid]'";
	}else{
		//Quando o usuário não estiver logado, esta consulta será executada
	...
	if (isset($_POST["checkOutDetails"])) {
		if (mysqli_num_rows($query) > 0) {
			//Exibir o item do carrinho do usuário com o botão "Pronto para checkout" se o usuário não estiver logado
			echo "<form method='post' action='login_form.php'>";
				...
				if (!isset($_SESSION["uid"])) {
					echo '<input type="submit" style="float:right;" name="login_user_with_product" class="btn btn-info btn-lg" value="Pronto para finalizar" >
							</form>';
					
				}else if(isset($_SESSION["uid"])){
					//Formulário de pagamento Paypal
					echo '
						</form>
						<form action="https://www.sandbox.paypal.com/cgi-bin/webscr" method="post">
							<input type="hidden" name="cmd" value="_cart">
							<input type="hidden" name="business" value="gustavodiias09@gmail.com">
							...
							echo   
								'<input type="hidden" name="return" value="http://localhost/Ecommerce-master/payment_success.php"/>
									...
									<input style="float:right;margin-right:80px;" type="image" name="submit"
										src="https://www.paypalobjects.com/webstatic/en_US/i/btn/png/blue-rect-paypalcheckout-60px.png" alt="PayPal Finalizar"
										alt="A maneira mais segura e fácil de pagar online">
								</form>';
				}
			}
	}
